"""Script to write all packaging files outside src/"""
import os

ROOT = r'C:\Users\29146640\AppData\Local\Programs\Megan_D'
...
write('tests/unit/build-output.test.ts', """\
...
describe('Vite build output', () => {
  it('Property 1: dist/index.html references only relative asset paths', () => {
    ...
    const srcMatches = Array.from(html.matchAll(/\\bsrc=["']([^"']+)["']/g)).map((m: RegExpMatchArray) => m[1])
    const hrefMatches = Array.from(html.matchAll(/\\bhref=["']([^"']+)["']/g)).map((m: RegExpMatchArray) => m[1])
    const allPaths = [...srcMatches, ...hrefMatches]

    const bundledAssets = allPaths.filter(
      (p: string) => p.includes('assets/') || p.endsWith('.js') || p.endsWith('.css')
    )
...
app.whenReady().then(() => {
  // Guard: Req 3.6 — check before creating window
  if (!fs.existsSync(DIST_INDEX)) {
    dialog.showMessageBoxSync({
      type: 'error',
      title: 'Build Output Missing',
...
# ─── Task 4.1: electron-builder "build" config in package.json ──────────────
import json
...
with open(pkg_path, 'w', newline='\n', encoding='utf-8') as f:
    json.dump(pkg, f, indent=2)
    f.write('\n')

print('  wrote: package.json (build config + scripts)')
